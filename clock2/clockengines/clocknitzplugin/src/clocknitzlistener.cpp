...
void CClockNitzListener::RunL()
	...
	if( iStatus == KErrNone ) 
	    {
	    // The check the network status before reading the NitzInfo
	    // has been removed because of timing problem between sysap and ClockNitzListener
	    // NET_TIME_IND comes before KCoreAppUIsNetworkConnectionAllowed 
	    // is set by sysap .So time does not get updated
