...
#define KVGALSCWidth 640
#define KVGALSCHeight 480
...
void CClockMainContainer::DrawMainViewText( CWindowGc& aWindowGc ) const
    ...
    if( Layout_Meta_Data::IsLandscapeOrientation() )
        {
        if (!CClockMainContainer::IsVGAMode())
            {
            ...
            layoutText.LayoutText(
                    mainClockPane,
                    AknLayoutScalable_Apps::main_clock2_pane_t1(3).LayoutLine());

            TPtr dayStringPtr = iDayNameString->Des();
            // Day long format.
            layoutText.DrawText(aWindowGc, *iDayNameString, ETrue,
                    cachedColor);

            // Get the layout for the datestring.
            layoutText.LayoutText(
                    mainClockPane,
                    AknLayoutScalable_Apps::main_clock2_pane_t2(3).LayoutLine());
            ...
            layoutText.LayoutText(
                    mainClockPane,
                    AknLayoutScalable_Apps::main_clock2_pane_t3(1).LayoutLine());
            ...
            AknTextUtils::DisplayTextLanguageSpecificNumberConversion(
                    timeZonePtr);
            // The timezone string.
            layoutText.DrawText(aWindowGc, *iTimeZoneString, ETrue,
                    cachedColor);

            // Get the layout for the country string.
            TAknTextComponentLayout labelLayout;
            labelLayout = AknLayoutScalable_Apps::main_clock2_pane_t4(1);
            AknLayoutUtils::LayoutLabel(iLocationLabel, parentRect.Rect(),
                    labelLayout.LayoutLine());
...
// ---------------------------------------------------------------------------
// CClockMainContainer::IsVGAMode()
// returns device screen supports VGA mode or not
// ---------------------------------------------------------------------------
//
 TBool CClockMainContainer::IsVGAMode()
    {
    CWsScreenDevice* dev = CCoeEnv::Static()->ScreenDevice();
    
    TPixelsAndRotation sizeAndRotation;

    dev->GetScreenModeSizeAndRotation(dev->CurrentScreenMode(),
            sizeAndRotation);

    if ((sizeAndRotation .iPixelSize.iWidth == KVGALSCWidth)
            && (sizeAndRotation .iPixelSize.iHeight == KVGALSCHeight))
        {
        return ETrue;
        }
    return EFalse;
