...
void CAlarmUtils::DoSilence()
    ...
    if( CanSnooze() )
        {
        TBool silenced( EFalse );
        if( IsCalendarAlarm() )
            {
            // do silencing only if alarm is being played and silencing possible
            if( iAlarmPlayer && !( IsRingingTypeSilent() || IsOffTone() ) )
                {
                silenced = ETrue;
                delete iAlarmPlayer;
                iAlarmPlayer = NULL;
