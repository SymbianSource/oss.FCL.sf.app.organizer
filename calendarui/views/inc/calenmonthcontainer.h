...
public:
    ...
    /**
     * Business Calendar- Container can request current variant layout index
     */
    TInt LayoutVariantIndexBc(TMonthLayoutVariant aLayout) const;
...
private:  // New variables
	...
	// When doing pinch zoom on month view, focus is changed to a random 
	// location. There is need to restore date and selected index that 
	// was previously selected(focused).	
	/// Own: previously selected(focused) date. 
	TTime iPreviousDate;
	/// Own: previously selected item index on month grid.
