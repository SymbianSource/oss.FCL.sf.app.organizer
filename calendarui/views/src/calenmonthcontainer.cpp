...
// ----------------------------------------------------------------------------
// CCalenMonthContainer::HandleDisplayPreviewPaneSettingsChange
// ----------------------------------------------------------------------------
//
void CCalenMonthContainer::HandleDisplayPreviewPaneSettingsChangeL()
	{
	// read calendar central repository to load "Display setting item" 
	// value.
	CRepository* repository = CRepository::NewL( KCRUidCalendar );
	CleanupStack::PushL( repository );
	TInt displayPreviewPane = 1;
	User::LeaveIfError( repository->Get( KCalendarDisplayPreviewPane, 
			displayPreviewPane ) );
	CleanupStack::PopAndDestroy( repository );
	
	// Create or delete preview pane depends on setting value.
	if ( !displayPreviewPane )
		{
		delete iPreview;
		iPreview = NULL;
		}
...
TInt CCalenMonthContainer::LayoutVariantIndex(TMonthLayoutVariant aLayout) const
    ...
    if ( FeatureManager::FeatureSupported( 
        ...
        {
        layoutVariant = LayoutVariantIndexBc( aLayout );
        }
    else
        {
...
// ----------------------------------------------------------------------------
//
TInt CCalenMonthContainer::LayoutVariantIndexBc( 
        TMonthLayoutVariant aLayout ) const
    {
    TRACE_ENTRY_POINT;

    TBitFlags layoutFlags = iLayoutManager->LayoutFlags();
    
    TBool infobarEnabled = 
            layoutFlags.IsSet( 
                CCalenContainerLayoutManager::ECalenContainerInfobar );
    
    TBool prevEnabled = 
            layoutFlags.IsSet( 
                CCalenContainerLayoutManager::ECalenContainerPreview );

    TBool weekNumsEnabled = UseWeeks();
    ...
    switch (aLayout)
        ...
        case ECaleMonthPaneG1:
            {
            if ( weekNumsEnabled && prevEnabled )
                layoutVariant = 8;
            else if ( !weekNumsEnabled && prevEnabled )
                layoutVariant = 9;
            else if ( weekNumsEnabled && !infobarEnabled )
                layoutVariant = 4;
            else if ( !weekNumsEnabled && !infobarEnabled )
                layoutVariant = 5;
            else if ( !weekNumsEnabled && infobarEnabled )
...
void CCalenMonthContainer::HandlePointerEventL(const TPointerEvent& aPointerEvent)
    ...
    if(AknLayoutUtils::PenEnabled())
        ...
        if ( iGestureControl )
			...
			if ( iMultiTouchStarted || iGestureHandled )
				{
				TRACE_EXIT_POINT;
				return;
				}
...
void CCalenMonthContainer::HandleListBoxEventL(
    ...
    switch ( aEventType )
        {
        // single tap is used in ReCal for switching to day view 
        case ( MEikListBoxObserver::EEventItemClicked ):
            {
            if ( !iView->MenuBar()->IsDisplayed() && !UsePreview() 
            		&& !iGestureHandled && !iMultiTouchStarted )
                {
                iServices.IssueCommandL( ECalenForwardsToDayView );
                }
            break;
            }
        // because of preview pane double tap is used in BC 
...
// 
// ----------------------------------------------------------------------------
void CCalenMonthContainer::HandleGestureL( const MGestureEvent& aEvent )
	{
	...
	switch( code )
		...
		case EGestureStart:
			...
			iPreviousDate = iDate;
			iPreviousItemIndex = iGrid->CurrentItemIndex();
			break;
			}
		...
		case EGestureMultiTouchStart:
			{
			iMultiTouchStarted = ETrue;
			break;
			}
		case EGestureMultiTouchReleased:
			{
			iMultiTouchStarted = EFalse;
			break;
			}
		case EGesturePinch:
			{
			if ( !iGestureHandled )
				...
				TInt pinchPercent = aEvent.PinchPercent();
				//pinch percent == 100 => no change in zoom
				//pinch percent < 0    => error
				if ( pinchPercent != 100 && pinchPercent > 0 )
					{
					//PinchPercent above 100 for zooming in
					//PinchPercent below 100 for zooming out
					if ( pinchPercent > 100 && iPreview )
						{
						TInt displayPreviewPane = 0;
						User::LeaveIfError( repository->Set( 
								KCalendarDisplayPreviewPane, 
								displayPreviewPane ) );
						delete iPreview;
						iPreview = NULL;
				...
				if ( iGestureHandled )
					{
					// Restore previous focused date
					TCalTime time;
					time.SetTimeLocalL( iPreviousDate );					
					iGrid->SetCurrentItemIndex( iPreviousItemIndex );					
					iServices.Context().SetFocusDateL( time,
							TVwsViewId( KUidCalendar, KUidCalenMonthView ) );					
					TMonth activeMonth( iDate.DateTime().Month() );
					TMonth previousMonth( iPreviousDate.DateTime().Month() );		
					iChangeMonth = EFalse;
					...
					HandleDayChangeL( iPreviousDate );
...
TRect CCalenMonthContainer::InfoBarRectL( TBool aToolbarAvailable )
    ...
    if ( FeatureManager::FeatureSupported( 
            KFeatureIdFfCalScrollablePreview  ) )
        {
        //infobar is not displayed in mirrored layout
        layoutVariant = 2;
        }
    else
        {
