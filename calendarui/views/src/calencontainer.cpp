...
void CCalenContainer::UpdateStatusPaneAndExtensionsL()
    ...
    //This dialog check is added for Business calenadar, to avoid the navipane 
    //in Business calendar dialogs.
    if(!iEikonEnv->EikAppUi()->IsDisplayingDialog())
        {        
...
TRect CCalenContainer::ReducePreview( TRect aRect ) const
    ...
    if ( FeatureManager::FeatureSupported( 
        ...
        TInt orientation = Layout_Meta_Data::IsLandscapeOrientation() ? 4 : 0;
...
// ----------------------------------------------------------------------------
// CCalenContainer::HandleDisplayPreviewPaneSettingsChangeL()
// when calendar settings changed.
// ----------------------------------------------------------------------------
void CCalenContainer::HandleDisplayPreviewPaneSettingsChangeL()
    {
    TRACE_ENTRY_POINT;
