...
void CCalenWeekContainer::SetActiveContextFromHighlightL(TBool aInstAvailable)
    ...
    if (row >= 0)
        ...
        if( itemInfo.HasInstance() && aInstAvailable && iViewPopulationComplete)
            ...
            CCalInstance* instance = itemInfo.iInstance;
			if( !IsTimedNoteL(instance->Entry()) )       // todo/anniv/memo
                {
...
void CCalenWeekContainer::SizeChanged()
    ...
    TInt layoutToolBarVariant = lay.LayoutVariantIndex(TWeekListBoxLayout::EListScrollCaleWeekPane);
    // Cached layout value:
...
// ----------------------------------------------------------------------------
// CCalenWeekContainer::HandleDisplayPreviewPaneSettingsChange
// ----------------------------------------------------------------------------
//
void CCalenWeekContainer::HandleDisplayPreviewPaneSettingsChangeL()
	{
...
// ----------------------------------------------------------------------------
// CCalenWeekContainer::HandleListBoxEventL
// ----------------------------------------------------------------------------
//
void CCalenWeekContainer::HandleListBoxEventL(
        CEikListBox* /*aListBox*/, TListBoxEvent aEventType )
    {
    switch ( aEventType )
        {
        //Single tap is handled in HandlePointerEventL
        ...
        case ( MEikListBoxObserver::EEventItemDoubleClicked ):
            ...
            if ( !iView->MenuBar()->IsDisplayed()
				&& FeatureManager::FeatureSupported(
						KFeatureIdFfCalScrollablePreview) 
				&& UsePreview() && !iGestureHandled && !iMultiTouchStarted 
				&& (iPreviousColumn == iColumn 
				&& iPreviousItemIndex == iListBox->View()->CurrentItemIndex()))
                {
                iServices.IssueCommandL( ECalenForwardsToDayView );
                }
            break;
...
// ----------------------------------------------------------------------------
// CCalenWeekContainer::HandleGestureL
// 
// ----------------------------------------------------------------------------
void CCalenWeekContainer::HandleGestureL( const MGestureEvent& aEvent )
	{
		...
		case EGesturePinch:
			...
			if ( !iGestureHandled )
				...
				if ( iGestureHandled )
					...
					iRow = iPreviousRow;
					TInt oldColumn = iColumn;
					iColumn = iPreviousColumn;
					iListBox->View()->SetCurrentItemIndex( iPreviousItemIndex );
				
					CalcDayFromColumn();
					SetActiveContextFromHighlightL();
					if ( iColumn != oldColumn )
						{
						UpdateStatusPaneAndExtensionsL();
						}
					//SizeChanged();
