...
CNSmlNotepadDataStore::~CNSmlNotepadDataStore()
	...
	if ( iNotepadModsFetcher )
	{
	iNotepadModsFetcher->Cancel(); 
	delete iNotepadModsFetcher;
	iNotepadModsFetcher = NULL;
	}
									...
									TRequestStatus& aStatus )
{
	...
	*iCallerStatus = KRequestPending;
	...
	iNotepadModsFetcher = CNSmlNotepadModsFetcher::NewL( iSnapshotRegistered, iNpdDb, iKey, *iChangeFinder );
	iNotepadModsFetcher->FetchModificationsL( aStatus );
...
void CNSmlNotepadDataStore::DoCancelRequest()
	...
	if(iNotepadModsFetcher && iNotepadModsFetcher->IsActive())
	{
		iNotepadModsFetcher->Cancel();
	}
...
CSmlDataStoreFormat* CNSmlNotepadDataStore::DoOwnStoreFormatL()
{
	...
	TFileName fileName;
	TParse parse;
	parse.Set( KNSmlNotepadStoreFormatRsc, &KDC_RESOURCE_FILES_DIR, NULL );
